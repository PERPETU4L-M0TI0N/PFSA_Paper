...
def mu_membrane(lam,E0,beta_HM):
    ...
    ############################    Charging   ###############################
    Is = (rho0/2*M0)*(1/x0)*(zH**2*xH)
    # Is = rho0*zH**2/(2*M0*lam)  #ionic strength 
    
    mu0_chg2 = ((zM)**2/(b*B*lam)) #membrane-specific term  
    mu0_chg = A*R*T*(((2/3)*Is**(3/2)*sigma_fun(B*a*Is**(1/2))*M0/rho0) + mu0_chg2)  
    ##########################################################################
    ...
    ############################    Solvation   ##############################
    def alpha0_cal(alpha0):       
        """solving the implicit alpha equation"""
        hs = Ns*ks*alpha0*x0/(1+ks*alpha0*x0)        
        alpha0_pred = (1 - (1/x0)*np.sum(hs*xs)) / (x0 + np.sum(xs*(1 - hs))) 
        return alpha0_pred - alpha0
    
    alpha0 = sp.optimize.root_scalar(alpha0_cal, bracket=[-0.1,1.1], method= 'brentq').root      
    
    mu0_solv = R*T*log(alpha0)
    ##########################################################################
    
    ##############################    Total   ################################
    mu0_mem =  mu0_id + mu0_phy +mu0_swe + mu0_chg + mu0_solv 
    ##########################################################################
...
def Res_Params_CurveFit(RH_exp, E0, beta_HM):
